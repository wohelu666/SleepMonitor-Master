...
public class SleepQualityView extends View {
    ...
    private int textGrayColor = Color.parseColor("#000000");//文字灰色
    //线条属性
    ...
    protected void onDraw(Canvas canvas) {
        if (getWidth() > 0 && getHeight() > 0) {
            //线条区域的高度    =   1f( //画板上边距百分比 - //画板下边距百分比  -  //x轴文字的大小百分比)*view 的高度
            float canvasHeight = (1f - marginTopRatio - marginBottomRatio - xTextRatio) * getHeight();
            //睡醒坐标点的y坐标值 =   //(y轴文字的大小百分比 + 画板上边距百分比)*view 的高度
            wakeYAxis = (yTextRatio + marginTopRatio) * getHeight();
            //浅睡坐标点的y坐标值 = //(y轴文字的大小百分比 + 画板上边距百分比)*view 的高度+线条区域的高度/3
            shallowYAxis = (yTextRatio + marginTopRatio) * getHeight() + canvasHeight / 3;
            //深睡坐标点的y坐标值 =/(y轴文字的大小百分比 + 画板上边距百分比)*view 的高度+ 2* 线条区域的高度/3
...
//            canvas.drawText(deepString, marginLeftRatio * getWidth(), deepYAxis, textPaint);
            canvas.drawText(wakeString, getWidth() - (marginLeftRatio * getWidth()), wakeYAxis, textPaint);
            canvas.drawText(shallowString, getWidth() - marginLeftRatio * getWidth(), shallowYAxis, textPaint);
            canvas.drawText(deepString, getWidth() - marginLeftRatio * getWidth(), deepYAxis, textPaint);
            float yWidth = getMaxYWidth(textPaint);
            //伸展动画
            float mPhaseX = lineAnimator.getPhaseX();
            //线条区域的宽度
            float canvasWidth = ((1f - marginLeftRatio - marginRightRatio - marginYRatio - yTextRatio) * getWidth() - yWidth) * mPhaseX;
            float xDistance = yWidth + (marginLeftRatio + marginYRatio) * getWidth();
            //画x坐标
...
//                textPaint.setTextAlign(Paint.Align.CENTER);
                for (int i = 0; i < xAxisArray.size(); i++) {
                    canvas.drawText(xAxisArray.get(i), xDistance + i * canvasWidth / xAxisArray.size(), (1 - marginBottomRatio) * getHeight(), textPaint);
                }
            }
            //画线
            if (this.timeArray != null && this.timeArray.size() > 0) {
                ...
                for (int i = 0; i < timeArray.size(); i++) {
                    ...
                    if (selectValue.length >= 3) {
                        ...
                        float startXAxis = xDistance + selectValue[1] * canvasWidth;//开始时间
...
//
//                            } else {
//                                canvas.drawLine(startXAxis - vWidth / 2 - 150, yAxis, endXAxis + vWidth / 2 - 150, yAxis, linePaint);
//
//                            }
//
//                        } else {
...
//                        }
                        if (i < timeArray.size() - 1) {
                            drawNext(canvas, linePaint, sleepType, endXAxis, yAxis, xDistance, canvasWidth, timeArray.get(i + 1), i);
                        }
                    }
                }
            }
     ...
     * 绘制两点间隔线
     */
    private void drawNext(Canvas canvas, Paint paint, float sleepType, float endXAxis, float yAxis, float xDistcance, float canvasWidth, float[] nextValue, int i) {
...
//
//        } else {
            canvas.drawLine(endXAxis - 150, yAxis, startNextXAxis - 150, yNextAxis, paint);
...
//        }
    ...
    private void init(AttributeSet attrs) {
        ...
        if (attrs != null) {
            //初始化布局属性
            TypedArray typedArray = getContext().getTheme().obtainStyledAttributes(attrs, com.yhd.sleepquality.R.styleable.SleepQualityView, 0, 0);
            marginLeftRatio = typedArray.getFloat(com.yhd.sleepquality.R.styleable.SleepQualityView_sq_marginLeftRatio, marginLeftRatio);
            marginRightRatio = typedArray.getFloat(com.yhd.sleepquality.R.styleable.SleepQualityView_sq_marginRightRatio, marginRightRatio);
            marginTopRatio = typedArray.getFloat(com.yhd.sleepquality.R.styleable.SleepQualityView_sq_marginTopRatio, marginTopRatio);
            marginBottomRatio = typedArray.getFloat(com.yhd.sleepquality.R.styleable.SleepQualityView_sq_marginBottomRatio, marginBottomRatio);
            widthRatio = typedArray.getFloat(com.yhd.sleepquality.R.styleable.SleepQualityView_sq_widthRatio, widthRatio);
            vWidthRatio = typedArray.getFloat(com.yhd.sleepquality.R.styleable.SleepQualityView_sq_vwidthRatio, vWidthRatio);
            marginYRatio = typedArray.getFloat(com.yhd.sleepquality.R.styleable.SleepQualityView_sq_marginYRatio, marginYRatio);
            xTextRatio = typedArray.getFloat(com.yhd.sleepquality.R.styleable.SleepQualityView_sq_xTextRatio, xTextRatio);
            yTextRatio = typedArray.getFloat(com.yhd.sleepquality.R.styleable.SleepQualityView_sq_yTextRatio, yTextRatio);
            animationTime = typedArray.getInteger(com.yhd.sleepquality.R.styleable.SleepQualityView_sq_animationTime, animationTime);
        }
        //初始化画笔
        linePaint = new Paint();
...
//        timeArray.add(new float[]{2f, 0.6f, 0.10f});
//        timeArray.add(new float[]{1f, 0.70f, 0.25f});
//////
//        xAxisArray.add("23");
//        xAxisArray.add("24");
//        xAxisArray.add("1");
